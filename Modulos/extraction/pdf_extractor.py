...
def procesar_carpeta(ruta_carpeta, nombre_archivo_json_global):
    ...
    for nombre_archivo in os.listdir(ruta_carpeta):
        if nombre_archivo.lower().endswith(".pdf"):
            ...
            ruta_archivo_json_individual = os.path.join(ruta_carpeta, nombre_archivo_json_individual)  # <-- Guardar en la carpeta de la materia
            ...
            try:
                with open(ruta_archivo_pdf, 'rb') as pdf_file:
                    ...
                    print(f"Se procesó el archivo: {nombre_archivo} y se guardaron sus preguntas en: {ruta_archivo_json_individual}")
    ...
    # Cambiado: guardar el JSON global en la misma carpeta de la materia
    ...
    print(f"\nSe procesaron todos los archivos PDF y todas las preguntas relevantes (incluyendo duplicados) se guardaron en: {ruta_archivo_json_global}")

def extraer_y_guardar_resumen_pdf(ruta_carpeta, nombre_materia):
    """
    Extrae el texto del PDF de resumen de la materia y lo guarda en un JSON.
    """
    resumen_pdf = None
    for nombre_archivo in os.listdir(ruta_carpeta):
        if nombre_archivo.lower().startswith('resumen') and nombre_archivo.lower().endswith('.pdf'):
            resumen_pdf = nombre_archivo
            break
    if not resumen_pdf:
        print(f"[ADVERTENCIA] No se encontró PDF de resumen en {ruta_carpeta}")
        return
    ruta_pdf = os.path.join(ruta_carpeta, resumen_pdf)
    texto = ""
    try:
        ...
        print(f"[ERROR] No se pudo extraer el resumen de {ruta_pdf}: {e}")
        return
    resumen_json = {
        "materia": nombre_materia,
        "resumen": texto.strip()
    }
    nombre_json = f"resumen_{nombre_materia.lower().replace(' ', '_')}.json"
    ruta_json = os.path.join(ruta_carpeta, nombre_json)
    with open(ruta_json, 'w', encoding='utf-8') as f:
        json.dump(resumen_json, f, indent=2, ensure_ascii=False)
    print(f"Resumen de {nombre_materia} guardado en {nombre_json}")
...
# Extraer y guardar resumen de cada materia
extraer_y_guardar_resumen_pdf(ruta_carpeta1, 'Ciencia_Datos')
extraer_y_guardar_resumen_pdf(ruta_carpeta2, 'Habilidades_Vida')