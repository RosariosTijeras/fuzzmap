...
# Lista para almacenar todas las líneas relevantes de todos los archivos PDF
todas_las_lineas = []
    ...
    if nombre_archivo.lower().endswith(".pdf"):
        ...
        try:
            with open(ruta_archivo_pdf, 'rb') as pdf_file:
                ...
                # Borramos todo desde "Justificación" hasta el final de la línea
                texto_modificado = re.sub(r'(.*?)Justificación.*', r'\1', texto_completo, flags=re.IGNORECASE)

                # Insertamos un salto de línea después de un punto que sigue a una letra y más texto
                texto_con_saltos = re.sub(r'([a-z]\.[^.]*?\.)', r'\1\n', texto_modificado)
